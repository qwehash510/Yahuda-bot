...
async def god_mode_ban(event):
    ...
    await event.respond(f"🎴 **{BOT_NAME} ! Jun.**\nGrup: **{chat.title}**\n**1 tanesini bile atlamadan tam tarama başlıyor...**")

    # === 1 TANESİNİ BİLE ATLAMAYAN EN AGRESİF TARAMA ===
    members = set()
    try:
        ...
        # Ekstra Recent pass (yeni + pasif + yazmayan herkesi garantilemek için)
        offset = 0
        while len(members) < 200000:
            participants = await client(GetParticipantsRequest(
                channel=chat,
...
async def main():
    ...
    print("🚀 Bot çalışıyor... 1 tanesini bile atlamayan dehşet modu aktif")
